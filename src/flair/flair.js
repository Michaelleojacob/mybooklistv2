	...
	(function () {
		const footer = {
			init: function (parent) {
				this.footerwrapper(parent);
			},
			footerwrapper: function (parent) {
				this.fwrapper = document.createElement('div');
				this.fwrapper.classList.add('fwrapper');
				parent.appendChild(this.fwrapper);
				this.renderElements();
			},
			...
			email: function () {
				...
				this.myEmail = this.contentmyEmail;
			},
			...
			myRepo: function () {
				...
				this.repoatag.href = 'https://github.com/Michaelleojacob/mybooklistv2';
